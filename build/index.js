...
class FeatBitMCPServer {
            ...
            try {
                switch (name) {
                    case 'configure_featbit':
                        return this.configureFeatBit(args);
                    case 'list_feature_flags':
                        return this.listFeatureFlags();
                    case 'get_feature_flag':
                        return this.getFeatureFlag(args);
                    case 'evaluate_feature_flag':
                        return this.evaluateFeatureFlag(args);
                    case 'evaluate_multiple_flags':
                        return this.evaluateMultipleFlags(args);
                    case 'get_all_flag_values':
                        return this.getAllFlagValues(args);
                    default:
    ...
    async listFeatureFlags() {
        ...
        const flagList = flags.map(flag => `🚩 **${flag.key}** (${flag.name})\n   Status: ${flag.isEnabled ? '✅ Enabled' : '❌ Disabled'}\n   Description: ${flag.description || 'No description'}`).join('\n\n');
        return {
            content: [
                {
                    type: 'text',
    ...
    async evaluateFeatureFlag(args) {
        if (!this.featbitClient) {
            throw new Error('FeatBit not configured. Please run configure_featbit first.');
        }
    ...
    async evaluateMultipleFlags(args) {
        if (!this.featbitClient) {
            throw new Error('FeatBit not configured. Please run configure_featbit first.');
        }
        ...
        const results = evaluations.map(evaluation => `🚩 **${evaluation.flagKey}**: ${JSON.stringify(evaluation.value)} ${evaluation.flagFound ? '✅' : '❌'}`).join('\n');
        return {
            content: [
                {
                    type: 'text',
    ...
    async getAllFlagValues(args) {
        ...
        const valueList = Object.entries(values).map(([key, value]) => `🚩 **${key}**: ${JSON.stringify(value)}`).join('\n');
        return {
            content: [
                {
                    type: 'text',
